...
population. 
On the other hand, the age group `r r summary_info$least_age` has the smallest percentage of 
...
The dataframe's column names (cdc_report_dt, Race and ethnicity (combined), hosp_yn, death_yn) has been modified to be more human readable. The dataframe has filtered out data that were not used (pos_spec_dt, onset_dt, icu_yn, medcond_yn). The dataframe was sorted in the ascending order of date and was grouped by current status.
...
The chart showing the hospital treatment rate among different race and 
...
the other hand, the age group `least_age` has the smallest percentage of confirmed 
cases that takes up only about `format(least_percent * 100, digits = 1 )`%. From 
